...
export default function (languageText, partsOfSpeechToGet) {
    ...
    return startsByPos.reduce((acc, posAndStart, i, all) => {
        ...
        acc[posAndStart.pos] = removeExtraneous(languageText.slice(posAndStart.start, end));
        return acc;
    }, {});
}

function removeExtraneous(text) {
    ...
    for (let i = 0; i < extraneousHeaders.length; i += 1) {
        if (text.indexOf(extraneousHeaders[i]) >= 0) {
            return text.slice(0, text.indexOf(extraneousHeaders[i]));
        }
    }
    return text;
}